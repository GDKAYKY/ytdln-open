...
const defaultSettings = {
  ...
  // New Settings from Implementation
...
function applySettingsToUI() {
  applyBasicSettingsToUI();
  applyAdvancedSettingsToUI();
  applyFolderSettingsToUI();
  applyNewDownloadSettingsToUI();
}

function applyBasicSettingsToUI() {
  ...
  document.getElementById("concurrentFragments").value =
    currentSettings.concurrentFragments;
  document.getElementById("concurrentFragmentsValue").textContent =
    currentSettings.concurrentFragments;
  document.getElementById("embedSubs").checked = currentSettings.embedSubs;
  document.getElementById("writeInfoJson").checked =
    currentSettings.writeInfoJson;
}

function applyAdvancedSettingsToUI() {
  ...
  // Use new ID for socketTimeout if only present in Downloads tab
  const stEl = document.getElementById("socketTimeout");
  if (stEl) stEl.value = currentSettings.socketTimeout;

  document.getElementById("retries").value = currentSettings.retries;
  document.getElementById("fragmentRetries").value =
    currentSettings.fragmentRetries;

  if (document.getElementById("extractorRetries"))
    document.getElementById("extractorRetries").value =
      currentSettings.extractorRetries;

  const ncEl = document.getElementById("noCheckCertificate");
  if (ncEl) ncEl.checked = currentSettings.noCheckCertificate;

  const ieEl = document.getElementById("ignoreErrors");
  if (ieEl) ieEl.checked = currentSettings.ignoreErrors;

  document.getElementById("writeThumbnail").checked =
    currentSettings.writeThumbnail;
  document.getElementById("writeDescription").checked =
    currentSettings.writeDescription;
  ...
  const restrictEl = document.getElementById("restrictFileName");
  if (restrictEl)
    restrictEl.checked =
      currentSettings.restrictFileName || currentSettings.restrictFilenames;

  if (document.getElementById("useSponsorBlock"))
    document.getElementById("useSponsorBlock").checked =
      currentSettings.useSponsorBlock;
  if (document.getElementById("embedMetadata"))
    document.getElementById("embedMetadata").checked =
      currentSettings.embedMetadata;
  if (document.getElementById("writeAutoSubs"))
    document.getElementById("writeAutoSubs").checked =
      currentSettings.writeAutoSubs;
}

function applyFolderSettingsToUI() {
  document.getElementById("musicFolder").value =
    currentSettings.musicFolder || "";
  document.getElementById("videoFolder").value =
    currentSettings.videoFolder || "";

  if (document.getElementById("videoFileNameModel"))
    document.getElementById("videoFileNameModel").value =
      currentSettings.videoFileNameModel;
  if (document.getElementById("musicFileNameModel"))
    document.getElementById("musicFileNameModel").value =
      currentSettings.musicFileNameModel;

  if (document.getElementById("cacheDownloadsFirst"))
    document.getElementById("cacheDownloadsFirst").checked =
      currentSettings.cacheDownloadsFirst;
  if (document.getElementById("noFragments"))
    document.getElementById("noFragments").checked =
      currentSettings.noFragments;
  if (document.getElementById("keepFragments"))
    document.getElementById("keepFragments").checked =
      currentSettings.keepFragments;
}

function applyNewDownloadSettingsToUI() {
  // ===== NEW DOWNLOAD TAB SETTINGS =====
  const els = [
    "anonymous",
    "showDownloadCard",
    ...
    "useCookies",
    "useAria2",
    "rememberDownloadType",
    "downloadRegistry",
  ];
  els.forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.checked = !!currentSettings[id];
  });

  if (document.getElementById("connectionLimit"))
    document.getElementById("connectionLimit").value =
      currentSettings.connectionLimit;
  if (document.getElementById("bufferSize"))
    document.getElementById("bufferSize").value = currentSettings.bufferSize;

  // Radio groups
  setRadioValue("preferredDownloadType", currentSettings.preferredDownloadType);
  setRadioValue(
    "avoidDuplicatedDownloads",
    currentSettings.avoidDuplicatedDownloads
  );
  setRadioValue(
    "cleanDownloadLeftovers",
    currentSettings.cleanDownloadLeftovers
  );

  // Sliders in Download tab
  if (document.getElementById("dl_concurrentFragments")) {
    document.getElementById("dl_concurrentFragments").value =
      currentSettings.concurrentFragments;
    document.getElementById("dl_concurrentFragmentsValue").textContent =
      currentSettings.concurrentFragments;
  }
  if (document.getElementById("concurrentDownloads")) {
    document.getElementById("concurrentDownloads").value =
      currentSettings.concurrentDownloads;
    document.getElementById("concurrentDownloadsValue").textContent =
      currentSettings.concurrentDownloads;
  }
}
...
function getSettingsFromUI() {
  return {
    ...getBasicSettingsFromUI(),
    ...getAdvancedSettingsFromUI(),
    ...getFolderSettingsFromUI(),
    ...getNewDownloadSettingsFromUI(),
  };
}

function getBasicSettingsFromUI() {
  return {
    outputFormat: document.getElementById("outputFormat").value,
    quality: document.getElementById("quality").value,
    audioFormat: document.getElementById("audioFormat").value,
    concurrentFragments: Number.parseInt(
      document.getElementById("concurrentFragments").value
    ),
...
function getAdvancedSettingsFromUI() {
  return {
    ...
    socketTimeout: Number.parseInt(
      document.getElementById("socketTimeout")?.value ||
        currentSettings.socketTimeout
    ),
    retries: Number.parseInt(document.getElementById("retries").value),
    fragmentRetries: Number.parseInt(
      document.getElementById("fragmentRetries").value
    ),
    extractorRetries: Number.parseInt(
      document.getElementById("extractorRetries")?.value ||
        currentSettings.extractorRetries
    ),
    noCheckCertificate:
      document.getElementById("noCheckCertificate")?.checked ??
      currentSettings.noCheckCertificate,
    ignoreErrors:
      document.getElementById("ignoreErrors")?.checked ??
      currentSettings.ignoreErrors,
    ...
    restrictFileName:
      document.getElementById("restrictFileName")?.checked ?? false,
    useSponsorBlock:
      document.getElementById("useSponsorBlock")?.checked ?? false,
    embedMetadata: document.getElementById("embedMetadata")?.checked ?? false,
    writeAutoSubs: document.getElementById("writeAutoSubs")?.checked ?? false,
  };
}

function getFolderSettingsFromUI() {
  return {
    musicFolder: document.getElementById("musicFolder").value,
    videoFolder: document.getElementById("videoFolder").value,
    videoFileNameModel:
      document.getElementById("videoFileNameModel")?.value || "%(title)s",
    musicFileNameModel:
      document.getElementById("musicFileNameModel")?.value || "%(title)s",
    cacheDownloadsFirst:
      document.getElementById("cacheDownloadsFirst")?.checked ?? false,
    noFragments: document.getElementById("noFragments")?.checked ?? false,
    keepFragments: document.getElementById("keepFragments")?.checked ?? false,
  };
}

function getNewDownloadSettingsFromUI() {
  return {
    anonymous: document.getElementById("anonymous")?.checked ?? false,
    showDownloadCard:
      document.getElementById("showDownloadCard")?.checked ?? true,
    fastDownload: document.getElementById("fastDownload")?.checked ?? false,
    useCookies: document.getElementById("useCookies")?.checked ?? false,
    useAria2: document.getElementById("useAria2")?.checked ?? false,
    connectionLimit: document.getElementById("connectionLimit")?.value ?? "",
    bufferSize: document.getElementById("bufferSize")?.value ?? "",
    preferredDownloadType: getRadioValue("preferredDownloadType") || "video",
    rememberDownloadType:
      document.getElementById("rememberDownloadType")?.checked ?? false,
    avoidDuplicatedDownloads:
      getRadioValue("avoidDuplicatedDownloads") || "none",
    cleanDownloadLeftovers: getRadioValue("cleanDownloadLeftovers") || "none",
    downloadRegistry:
      document.getElementById("downloadRegistry")?.checked ?? false,
    concurrentDownloads: Number.parseInt(
      document.getElementById("concurrentDownloads")?.value ?? "1"
    ),
  };
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (window.electronAPI) {
    ...
    console.error(
      "CRITICAL: window.electronAPI is not defined. Preload script might have failed."
    );
    updateStatus(
      "Erro Crítico: API do Electron não carregada. Verifique o console."
    );
  }
});
...
function setupModalEvents() {
  // Helper para fechar modal com animação
  const closeSettings = () => {
...
function setupControlEvents() {
  // Sync range inputs with their value displays and other range inputs if they have data-sync
  ...
  rangeInputs.forEach((input) => {
    input.addEventListener("input", (e) => {
      ...
      // Update its own value display
      const valueSpan = document.getElementById(`${id}Value`);
      if (valueSpan) valueSpan.textContent = value;

      // Sync with other range inputs if they have data-sync or share the same sync target
      const syncTarget = e.target.getAttribute("data-sync") || id;
      document
        .querySelectorAll(
          `input[type="range"][data-sync="${syncTarget}"], input[type="range"]#${syncTarget}`
        )
        .forEach((other) => {
          if (other !== e.target) {
            other.value = value;
            const otherValueSpan = document.getElementById(`${other.id}Value`);
            if (otherValueSpan) otherValueSpan.textContent = value;
          }
        });
    });
  });
...
// Configurar eventos de pastas
function setupFolderEvents() {
  const musicFolderInput = document.getElementById("musicFolder");
  const videoFolderInput = document.getElementById("videoFolder");

  // Music Folder Controls
  document
    .getElementById("selectMusicFolderBtn")
    .addEventListener("click", () => {
      window.electronAPI.selectFolder("music");
    });

  document
    .getElementById("openMusicFolderBtn")
    .addEventListener("click", () => {
      window.electronAPI.openSpecificFolder(currentSettings.musicFolder);
    });

  // Video Folder Controls
  document
    .getElementById("selectVideoFolderBtn")
    .addEventListener("click", () => {
      window.electronAPI.selectFolder("video");
    });

  document
    .getElementById("openVideoFolderBtn")
    .addEventListener("click", () => {
      window.electronAPI.openSpecificFolder(currentSettings.videoFolder);
    });

  // Listen for folder selection
  window.electronAPI.onFolderSelected((data) => {
    if (data.type === "music") {
      musicFolderInput.value = data.path;
      currentSettings.musicFolder = data.path;
    } else if (data.type === "video") {
      videoFolderInput.value = data.path;
      currentSettings.videoFolder = data.path;
    }
    // Save settings immediately when folder is changed
    saveSettings();
  });

  // Temp Files Tools
  const moveTempDb = document.getElementById("moveTempFilesToDownloadsFolder");
  if (moveTempDb) {
    moveTempDb.addEventListener("click", () => {
      if (confirm("Move all temporary files to downloads folder?")) {
        window.electronAPI.moveTempFilesToDownloads();
      }
    });
  }

  const cleanTempBtn = document.getElementById("cleanTempFiles");
  if (cleanTempBtn) {
...
window.electronAPI.onBinariesStatus((data) => {
  ...
  if (data.status === "ready") {
    const appModeInfo = data.appMode
      ? ` | Modo: ${data.appMode.description}`
      : "";
    updateStatus(
      `Binários prontos! SO: ${data.platform} (${data.arch})${appModeInfo}`
    );
    ...
    if (loader) {
      ...
      if (globalThis.stopParticleLoader) {
        globalThis.stopParticleLoader();
      }
    }
...
const downloadPopupProgressFill = document.getElementById(
  "downloadPopupProgressFill"
);
const downloadPopupProgressText = document.getElementById(
  "downloadPopupProgressText"
);
...
function showDownloadPopup() {
  ...
  // Reset values
  if (downloadPopupFilename)
    downloadPopupFilename.textContent = "Preparing download...";
...
function sortFiles(files, sortOption) {
  ...
  switch (sortOption) {
    ...
    case "date-asc":
      sorted.sort(
        (a, b) => new Date(a.downloadDate) - new Date(b.downloadDate)
      );
      break;
    case "name-asc":
  ...
  if (sortMenu) {
    sortMenu.addEventListener("wheel", (e) => {
      ...
      const currentIndex = itemsArray.findIndex(
        (item) => item.dataset.value === currentSortOption
      );
...
function addFileActionListeners() {
  ...
  const deleteIcons = document.querySelectorAll(
    '.delete-icon[data-action="delete"]'
  );
...
deletePopup
  ?.querySelector(".delete-popup-overlay")
  ?.addEventListener("click", () => {
    hideDeletePopup();
  });
...
function addNewDownloadFile(file) {
  ...
  setTimeout(() => {
    const newItem = downloadsContainer.querySelector(
